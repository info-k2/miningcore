    ...
    public static double? Update(WorkerContextBase context, VarDiffConfig options, IMasterClock clock)
    {
        ...
        try
        {
            ...
            if(ctx.LastTs.HasValue)
            {
                ...
                // Possible New Diff (quantized to power-of-two steps)
                var rawDiff = difficulty * options.TargetTime / avg;
                if(rawDiff <= 0)
                    return null;

                var power = Math.Round(Math.Log(rawDiff, 2));
                var newDiff = Math.Pow(2, power);
