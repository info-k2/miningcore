...
public class BitcoinPool : PoolBase
{
    ...
    protected override async Task OnVarDiffUpdateAsync(StratumConnection connection, double newDiff, CancellationToken ct)
    {
        ...
        if(connection.Context.ApplyPendingDifficulty())
        {
            ...
            // after a difficulty change, immediately send a new job
            // and force miners to drop old work by setting clean_jobs = true
            if(currentJobParams is object[] jobParams && jobParams.Length > 0)
            {
                var jobParamsWithClean = (object[]) jobParams.Clone();

                // last parameter is clean_jobs flag in the Stratum mining.notify payload
                if(jobParamsWithClean[^1] is bool)
                    jobParamsWithClean[^1] = true;
                else
                    jobParamsWithClean = jobParamsWithClean.Concat(new object[] { true }).ToArray();

                await connection.NotifyAsync(BitcoinStratumMethods.MiningNotify, jobParamsWithClean);
            }
            else
            {
                await connection.NotifyAsync(BitcoinStratumMethods.MiningNotify, currentJobParams);
